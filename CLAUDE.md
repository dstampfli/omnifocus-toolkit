...
As with the sync script, config is compile-time constants at the top of the file (`MODEL`, `MOVE_MIN_CONFIDENCE`, `CHUNK_SIZE`), and all cross-process communication is JSON over `osascript`'s argv/stdout. `MODEL` defaults to `claude-haiku-4-5`. Unlike the sync script, the pure decision/parsing/prompt-building/reporting logic (everything except the two JXA calls and the live API call) is unit-tested under `tests/` — this includes `chunk_items`, but not `classify_in_batches`, which is thin I/O orchestration over the untested `classify`.
