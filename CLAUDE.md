...
- `omnifocus_mcp_server.py` is a local **stdio** MCP server (FastMCP / `mcp[cli]`) exposing three tools — `triage_inbox`, `review_tasks`, and read-only `omnifocus_status` — to a scheduled Claude Cowork task in Claude Desktop. It reuses the toolkit modules directly: `triage_inbox`/`review_tasks` call `run_triage`/`run_review`, and `omnifocus_status` calls `read_omnifocus`. `apply` defaults false; each tool wraps failures into `{"error": ...}`. No network/auth/tunnel — Claude Desktop launches it locally over stdio.
